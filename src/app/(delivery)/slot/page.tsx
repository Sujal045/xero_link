      ...
      <header className="sticky top-0 z-10 px-5 pt-6 pb-4 bg-slate-950/90 backdrop-blur-xl border-b border-white/5">
        ...
        <div className="flex gap-3 mt-4">
          <div className="flex-1 rounded-2xl bg-blue-500/10 px-4 py-3 text-center">
            <p className="text-lg font-bold text-blue-400">{orders.length}</p>
          ...
          Object.entries(slots).map(([slot, slotOrders]) => (
            <section key={slot}>
              <div className="flex items-center gap-2 mb-3 px-1">
                ...
                <h2 className="text-xs font-semibold text-blue-400 uppercase tracking-widest">{slot} Slot</h2>
              ...
              <div className="space-y-3">
                {slotOrders.map(order => (
                  <Link key={order.id} href={`/deliver?orderId=${order.id}`}>
                    <div className="group rounded-2xl border border-white/8 bg-white/4 hover:bg-white/8 hover:border-blue-500/20 p-4 transition-all duration-200 cursor-pointer">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right shrink-0">
                          <p className="font-bold text-blue-400">₹{Number(order.total_price).toFixed(0)}</p>
                          <p className="text-xs text-slate-500 mt-0.5">{order.total_pages} pgs</p>
                          <ChevronRight className="h-4 w-4 text-slate-600 group-hover:text-blue-400 ml-auto mt-1 transition-colors" />
                        </div>
                      </div>
                    </div>
                  </Link>
