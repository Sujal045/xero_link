      ...
      <div className="px-5 py-6 space-y-4 max-w-md mx-auto">
        ...
        {verified ? (
          <div className="flex flex-col items-center text-center py-12 space-y-4">
            ...
            <div className="rounded-2xl bg-emerald-500/10 border border-emerald-500/20 px-8 py-5 w-full">
              <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Collect from student</p>
              <p className="text-3xl font-bold text-emerald-400">₹{Number(order.total_price).toFixed(2)}</p>
            </div>
            <Button onClick={() => router.push('/slot')}
              className="w-full h-12 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-white font-semibold text-base">
              Back to Slot
            </Button>
          </div>
        ) : (
          <>
            ...
            <div className="rounded-2xl bg-white/4 border border-white/8 p-4 space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {order.users?.phone && (
                    <a href={`tel:${order.users.phone}`} className="flex items-center gap-1.5 text-xs text-slate-400 mt-0.5 hover:text-emerald-400 transition-colors">
            ...
            <div className="rounded-2xl bg-white/4 border border-white/8 p-4">
              ...
              <div className="flex items-start gap-2">
                <MapPin className="h-4 w-4 text-emerald-400 shirnk-0 mt-0.5" />
            ...
            <div className="rounded-2xl bg-white/4 border border-white/8 p-4 space-y-2">
              ...
              <div className="border-t border-white/10 pt-3 flex justify-between font-bold">
                ...
                <span className="text-emerald-400 text-lg">₹{Number(order.total_price).toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-white/4 border border-white/8 p-5">
              ...
              <div className="flex gap-2 mb-2">
                {[0,1,2,3,4,5].map(i => (
                  <div key={i} className={`flex-1 h-12 rounded-xl flex items-center justify-center text-xl font-bold border transition-all ${
                    otp[i] ? 'border-emerald-500/50 bg-emerald-500/10 text-emerald-400' : 'border-white/10 bg-black/20 text-slate-700'
                  }`}>
                    {otp[i] ?? '·'}
                  </div>
                ))}
              ...
              <input
                ...
                className="w-full mt-3 px-4 py-3 rounded-xl bg-black/20 border border-white/10 text-white text-center text-2xl font-mono tracking-widest focus:outline-none focus:border-emerald-500/50 placeholder:text-slate-700 placeholder:text-sm placeholder:tracking-normal"
              />
            </div>
            ...
            <Button
              ...
              className="w-full h-14 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 text-white font-bold text-base shadow-xl shadow-emerald-500/20 disabled:opacity-40"
            >
              {verifying
